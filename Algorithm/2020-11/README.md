...
- [BJ1717.cpp](https://github.com/Seulwoo/DailyStudy/blob/main/Algorithm/2020-11/BJ1717.cpp)
    골드4난이도 문제, divide&conquer의 대표 문제였던 것 같다. 옛날에 풀이를 본 기억이 있는데 그대로는 아니고 되살려서 풀어보았다. 집합의 root ( 가장 작은 값 ) 을 가리키는 group 배열을 선언하고, 초기에는 자기 자신만을 포함하는 집합을 이루고 있으므로 인덱스값으로 초기화한다. 합집합 연산을 할때마다 group 배열을 업데이트하고, 확인하는 연산을 하면 서로의 root 값이 같은지 확인한다.
